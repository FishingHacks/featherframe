...
class App {
    ...
    constructor(path) {
        ...
        if (this.#config.customHTML) {
            this.#html = this.#config.customHTML
        ...
        if (this.#config.launch)
            console.error(
                redBright(
                    'Auto Launch of additional programs is not yet implemented :<'
                )
            );
    }
    get path() {
...
async function loadApp(path, express, config = { debugprint: false }) {
    app = new App(path);
    await require('../framework/router.js').route(app, express, config);
    return app;
}

module.exports = {
    loadApp,
    build: require('../framework/router').build,
};
